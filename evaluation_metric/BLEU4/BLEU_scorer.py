...
def cook_refs(refs, eff=None, n=4):
    ...
    for ref in refs:
        ...
        reflen.append(rl)
        for (ngram, count) in counts.items():  # 修改了iteritems为items
...
def cook_test(test, reflen, refmaxcounts, eff=None, n=4):
    ...
    result['correct'] = [0]*n
    for (ngram, count) in counts.items():  # Python 3.x 使用 items() 而非 iteritems()
...
class BleuScorer(object):
    ...
    def compute_score(self, option=None, verbose=0):
        ...
        if verbose > 0:
            print(totalcomps)  # 用括号包裹内容
            print("ratio:", ratio)  # 用括号包裹内容，并且语法上正确
